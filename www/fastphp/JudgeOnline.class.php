...
class JudgeOnline /*implements DataBase*/
{
    ...
    protected function getSelectSql($columnarray = array(), $table, $where = "", $extra = "")
    {
        $column = "";
    ...
    public function select($columnarray = array(), $table, $where = "", $extra = "")
    {
        ...
        {
            $this->connect();
            $result = $this->mysql->query($this->getSelectSql($columnarray, $table, $where, $extra));
     ...
     * select函数需注意的地方，该函数也许注意
     * 注意：因本函数使用columnarray第一个键所代表的值进行索引，
     *      请保证这个键所对应的值唯一
     *      例:columnarray('UId', 'Passwd') 以UId所对应的值为索引，可行
     *         columnarray('Passwd', 'UId') 以passwd对应的值为索引，不可行(密码可能存在重复的情况)
     */
    public function selectReturnSpecialIndex($columnarray = array(), $table, $where = "", $extra = "")
    {
        try
        {
            $this->connect();
